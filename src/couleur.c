...
couleur_compteur *compte_couleur(couleur *c, int csize) {
  ...
  /*
   * créer une table de hachage pour stocker les différentes couleurs et leur
   * nombre
   */
  hcreate(csize);
  ...
  for (i = 0; i < csize; i++) {
    ...
    if (bc == BITS24) {
      ...
      sprintf(key, "%hd:%hd:%hd:%hd", c->c.c32[i].rouge, c->c.c32[i].vert,
...
void print_couleur_compteur(couleur_compteur *ccompteur) {
  ...
  for (i = 0; i < ccompteur->size; i++) {
    if (ccompteur->compte_bit == BITS24) {
      ...
      printf("%5hx %5hx %5hx %5hx: %10d\n", ccompteur->cc.cc32[i].c.rouge,
             ccompteur->cc.cc32[i].c.vert, ccompteur->cc.cc32[i].c.bleu,
             ccompteur->cc.cc32[i].c.alpha, ccompteur->cc.cc32[i].compte);
    }
  }
}
 ...
 */
static int compare_compteur(const void *ccp1, const void *ccp2,
                            void *compte_bitp) {
