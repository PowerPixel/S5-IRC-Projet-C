...
couleur_compteur *
compte_couleur (couleur * c, int csize)
{
  ...
  for (i = 0; i < csize; i++)
    {
      ...
      if (bc == BITS24)
	{
	  sprintf (key, "%hd:%hd:%hd", c->c.c24[i].rouge, c->c.c24[i].vert,
		   c->c.c24[i].bleu);
	}
      else if (bc == BITS32)
	{
	  sprintf (key, "%hd:%hd:%hd:%hd", c->c.c32[i].rouge,
		   c->c.c32[i].vert, c->c.c32[i].bleu, c->c.c32[i].alpha);
	}
      e.key = key;
...
void
print_couleur (couleur * c, int csize)
{
  int i = 0;
  ...
  for (i = 0; i < csize; i++)
    {
      if (c->compte_bit == BITS24)
	{			// 3 octets (RGB)
	  printf ("%5x %5x %5x\n", c->c.c24[i].rouge, c->c.c24[i].vert,
		  c->c.c24[i].bleu);
	}
      else if (c->compte_bit == BITS32)
	{			// 4 octets (RGBA)
...
void
print_couleur_compteur (couleur_compteur * ccompteur)
{
  int i = 0;
  ...
  for (i = 0; i < ccompteur->size; i++)
	...
	{
	  printf ("%5hx %5hx %5hx: %10d\n", ccompteur->cc.cc24[i].c.rouge,
		  ccompteur->cc.cc24[i].c.vert, ccompteur->cc.cc24[i].c.bleu,
		  ccompteur->cc.cc24[i].compte);
	}
      else if (ccompteur->compte_bit == BITS32)
	{
	  printf ("%5hx %5hx %5hx %5hx: %10d\n",
		  ccompteur->cc.cc32[i].c.rouge, ccompteur->cc.cc32[i].c.vert,
		  ccompteur->cc.cc32[i].c.bleu, ccompteur->cc.cc32[i].c.alpha,
		  ccompteur->cc.cc32[i].compte);
	}
    }
}
 ...
 */
static int
compare_compteur (const void *ccp1, const void *ccp2, void *compte_bitp)
{
...
void
trier_couleur_compteur (couleur_compteur * ccompteur)
{
  COMPTEBIT bc = ccompteur->compte_bit;
  if (ccompteur->compte_bit == BITS24)
    {
      qsort_r (ccompteur->cc.cc24, ccompteur->size,
	       sizeof (couleur24_compteur), compare_compteur, (void *) &bc);
    }
  else if (ccompteur->compte_bit == BITS32)
    {
      qsort_r (ccompteur->cc.cc32, ccompteur->size,
	       sizeof (couleur32_compteur), compare_compteur, (void *) &bc);
    }
}
