  ...
  while (*input != '\0')
	...
	case '}':
	  if (in_object)
	    {
	      perror
		("Object ended without starting (missing opening brace for object)");
	      exit (1);
	    }
	  break;
	case ' ':
...
void
print_json_object (JSONObject * object)
{
  printf ("{\n");
...
JSONField *
get_value (JSONObject * object, char *key)
{
  for (int i = 0; i < object->nb_field; i++)
    {
...
JSONObject *
create_json_object (char *code, JSONArray * arguments)
{
  JSONObject *object = malloc (sizeof (JSONObject));
...
void
insert_str_into_array (char *str, JSONArray * array)
{
  array->array_size += 1;
...
void
insert_int_into_array (int *integer, JSONArray * array)
{
  array->array_size += 1;
...
void
convert_to_data (char *data, JSONObject * object)
{
...
void
convert_json_array_to_data (char *data, JSONArray * array)
{
  sprintf (data + strlen (data), "[");
  for (int i = 0; i < array->array_size; i++)
    {
...
int
json_to_text (JSONObject * object, char *data)
{
...
int
json_to_text_calcul (JSONObject * object, char *data)
{
  if (object->nb_field != 2)
    {
      return -1;
